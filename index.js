...
Lexxy.sendMessage(from, "hayo ngomongin owner gua ya", text)
Lexxy.sendMessage("6285735980188@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
Lexxy.sendMessage(from, "Mampus Udh Ku Laporin", text)
}
if(budy.includes('rasya')){
Lexxy.sendMessage(from, "hayo ngomongin owner gua ya", text)
Lexxy.sendMessage("6285735980188@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
...
${simbol} ${prefix}tiktokaudio <link>
...
Lexxy.sendMessage(from, image, { quoted: mek, caption: `Random Cecan ${command}`})
break
case 'malaysia':
...
case 'getdeskgc':
				...
				if (!isGroup) return reply(mess.only.group)
					anu = from
			   metadete = await Lexxy.groupMetadata(anu)
				Lexxy.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
					case 'getbio':
            
	  var yy = mek.message.extendedTextMessage.contextInfo.participant
var p = await Lexxy.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
...
const sname = Lexxy.contacts[ambl] != undefined ? Lexxy.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : Lexxy.contacts[ambl].notify || Lexxy.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
				case 'getpp':
					anu = from
		...
		try {
					ppimg = await Lexxy.getProfilePicture(anu)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				ano = await Lexxy.getProfilePicture(anu)
				buffer = await getBuffer(ano)
				Lexxy.sendMessage(from, buffer, image, {quoted: mek})
		} else {
		}
			  break
				case 'getpict':
				...
				if (mek.message.extendedTextMessage != undefined){
					...
					try {
						pic = await Lexxy.getProfilePicture(mentioned[0])
					} catch {
						pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
					}
					thumb = await getBuffer(pic)
					Lexxy.sendMessage(from, thumb, MessageType.image)
				}
				break
       case 'hentai':
       case 'bokep':
       case 'randombokep':
       case 'nekopoi':
       reply(` وَلَا تَقْرَبُوا۟ ٱلزِّنَىٰٓ ۖ إِنَّهُۥ كَانَ فَٰحِشَةً وَسَآءَ سَبِيلً
...
Lexxy.sendMessage(from, omkeh, MessageType.audio, { quoted: mek, mimetype: 'audio/mp4', ptt: true })
    break
case 'sound7':
reply2('Sudah Dihapus karena telah melanggar pedoman komunitas kami')
break
case 'store':
...
break
	...
	Lexxy.sendMessage(from,nowmm ,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
		})
		})
		break 
case 'bego':
...
medial = await Lexxy.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
Lexxy.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'gemuk':
					...
					mediaz = await Lexxy.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					Lexxy.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'balik':
	...
	mediau = await Lexxy.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
					...
					mediao = await Lexxy.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
...
break
case 'donasi':
reply2('pulsa: 6285735980188 atau: 6285815212637')
break
case 'hidetag':   
if (!isGroup) return sticGroup(from)
...
break
            case 'tomp3':
					...
					mediad = await Lexxy.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						Lexxy.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 359996400, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
case 'herodetail':
...
case 'dadu':
		...
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			Lexxy.sendMessage(from, damdu, sticker, {quoted: mek})
			break
case 'pubg':
case 'ucpubg':
...
break
				...
				if (!isQuotedImage) return reply('reply imagenya!')
					...
					mediau = await Lexxy.downloadAndSaveMediaMessage(enmediau)
					await Lexxy.updateProfilePicture(botNumber, mediau)
					reply('Sukses')
					break
case 'setppgrup': 
case 'setpp':
...
break;
	        ...
	        if (!isGroup) return sticGroup(form)
...
const btngrass = {
    contentText: `${anu}`,
    footerText: '*_©Patrick_*',
    buttons: punten,
    headerType: 1
}
await Lexxy.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
				case 'setnamegc':
					...
					if (!isGroupAdmins) return sticAdmin(from)
              if (!isBotGroupAdmins) return sticBotadmin(from)
					Lexxy.groupUpdateSubject(from, `${body.slice(11)}`)
				...
				case 'setdescgc':
					...
					if (!isGroupAdmins) return sticAdmin(from)
                   if (!isBotGroupAdmins) return sticBotadmin(from)
					Lexxy.groupUpdateDescription(from, `${body.slice(10)}`)
...
case 'toimage':
              ...
              media_nya = await Lexxy.downloadAndSaveMediaMessage(encmedia_nya)
              ...
              Lexxy.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
              ...
              break
					...
					case 'turu':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				return Lexxy.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
case 'searchgrup':
...
Lexxy.sendMessage('6285735980188@s.whatsapp.net', options, text, {quoted: flexx})
Lexxy.sendMessage('6285735980188@s.whatsapp.net', options, text, {quoted: flexx})
...
break
					...
					case 'swm':
						...
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							ppp = `${args.join(' ')}`
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await Lexxy.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							...
							await ffmpeg(`${media}`)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/stickwm_${sender}.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											Lexxy.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				...
				for (let i = 0; i < argzi[1]; i++){
					Lexxy.sendMessage('status@broadcast', argzi[0], MessageType.text)
                    }
                    break	
				case 'upswteks':
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(10)
                    Lexxy.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    Lexxy.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`Patrick`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await Lexxy.downloadMediaMessage(encmedia)
						Lexxy.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await Lexxy.downloadMediaMessage(encmedia)
						Lexxy.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await Lexxy.downloadMediaMessage(encmedia)
						Lexxy.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					...
					var mediap = await Lexxy.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    Lexxy.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					...
					media = await Lexxy.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    Lexxy.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    Lexxy.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
...
reply(`Kirim gambar/video(minimal kurang dari 10 detik) dengan caption ${prefix}sticker`)
}
break
                    case 'exif':
                    if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
